...
export type When<T, V> = {
  is:
  <I>(expr: T, value: ((expr?: T) => I) | I) => When<T, V | I>,

  match:
  <I>(expr: RegExp, value: ((expr?: T) => I) | I) => When<T, V | I>,

  else:
  <E>(value: ((expr?: T) => E) | E) => V | E
}
...
const when = <T>(expr: T) => ({
  is:
  <V>(constExpr: T, value: ((expr?: T) => V) | V): When<T, V> =>
    expr === constExpr ?
      resolvedWhen(
        value instanceof Function ? value(expr) : value
      )
      : when(expr),
